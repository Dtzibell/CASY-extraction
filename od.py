...
while True:
    try:
        ...
        for df in medium_data.iter_slices(measurement_no):
            growth_rates = (df.with_columns(pl.col("OD (600 nm)").log().alias("log expr"))
                            .with_row_index("time").drop_nans().drop_nulls())
            average_growth_rate = np.polyfit(growth_rates["time"], growth_rates["log expr"], 1)
            average_growth_rate = ln(2) / average_growth_rate[0]
            strain = growth_rates["file_name"][-1][0:3+strain_name_length]
            plotting_data[strain].append(average_growth_rate)
    
        sheet_id += 1
    except ValueError:
        # prints could not determine dtype for column 1 if sheet_id = 0
        # apparently fastexcel indexes sheet_id starting 1
        break
...
for idx, key in enumerate(plotting_data.keys()):
    ...
    if (idx + 1) % 3 == 0:
        ...
        plt.legend()
        plt.savefig(f"OD/OD_{mediums[key[:3]]}.png", bbox_inches = "tight")
        plt.yscale("log")
        plt.savefig(f"OD/OD_{mediums[key[:3]]}_log.png", bbox_inches = "tight")
        plt.close()
